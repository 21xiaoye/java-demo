...
public class Trie {
    ...
    /*target表示待查找的目标字符串集合*/
    public Trie(List<String> target){
        root = new Node();
        this.target = target;
        buildTrieTree();
        build_AC_FromTrie();
    ...
    private void buildTrieTree(){
        for(String targetStr : target){
            ...
            curr.str = targetStr;
            curr.list.add(targetStr.length());
        }
    }
                ...
                if(p.table[i] != null){
                    ...
                    while(true){
                        ...
                        if(failTo.table[i] != null){
                            ...
                            if(p.table[i].fail.isWord()) p.table[i].list.addAll(p.table[i].fail.list);
                            break;
                        }else{/*继续向上寻找*/
                            failTo = failTo.fail;
                        }
    ...
    public String findReplae(String text){
        ...
        while (index < text.length()){
            ...
            if(curr.table[ch]!=null){
                curr = curr.table[ch];
                if(curr.isList() || curr.isWord()){
                    for (int num : curr.list){
                        String substring = text.substring(index - num + 1, index+1);
                    }
                }
                index++;
            }else{
    ...
    public static void main(String[] args){
        ...
        String text = "bcabcdebcedfabcdefababkabhabkcdfkcdf";

        Trie aca = new Trie(target);
        ...
        String replae = aca.findReplae(text);
    }
}
